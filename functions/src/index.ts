...
export const createAuthUser = functions.https.onCall(async (data: CreateAuthUserData, context: CallableContext) => {
  functions.logger.info("createAuthUser function triggered.", { structuredData: true });

  // 1. Authentication Check: Ensure the caller is authenticated.
  // context.auth will be undefined if the user is not authenticated.
  if (!context.auth) {
  ...
  const callerUid = context.auth.uid; // Now correctly accessed
  ...
  try {
    ...
    functions.logger.info(`createAuthUser: Fetched admin document for UID ${callerUid}. Role found: '${adminRole}'. Document data:`, adminData);
    ...
    functions.logger.info(`createAuthUser: Caller UID ${callerUid} confirmed as admin. Proceeding...`);
     ...
     functions.logger.error(`createAuthUser: Error during admin role verification for UID ${callerUid}:`, err);
  ...
  // 3. Input Data Validation (using the strongly-typed 'data' parameter)
  const { email, password, displayName } = data; // Now correctly typed
  ...
  if (!email || typeof email !== 'string' || !email.includes('@')) {
    functions.logger.error("createAuthUser: Invalid or missing email.", { email });
    throw new functions.https.HttpsError("invalid-argument", "A valid email address is required.");
  }
  if (!password || typeof password !== 'string' || password.length < 6) {
    functions.logger.error("createAuthUser: Invalid or missing password (must be string, >= 6 chars).", { passwordLength: password?.length, passwordType: typeof password });
  ...
  if (!displayName || typeof displayName !== 'string' || displayName.trim().length < 2) {
    functions.logger.error("createAuthUser: Invalid or missing displayName (must be string, >= 2 chars).", { displayName });
  ...
  try {
    ...
    functions.logger.error(`createAuthUser: Error from admin.auth().createUser() for email ${email}:`, error);
