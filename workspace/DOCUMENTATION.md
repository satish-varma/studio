...
*   **Data Export (CSV):**
    ...
    *   Import data from CSV files for stock items and food expenses.
*   **Food Stall Settings:**
    *   Manage a centralized list of food vendors.
*   **Danger Zone (Admin Only):**
    *   Reset all application data (excluding user accounts).
...
*   **`/csv-import/route.ts`:**
    *   **Method:** `POST`
    *   **Purpose:** Allows an authenticated admin to import data from a CSV file.
    *   **Auth:** Requires a Bearer token (Firebase ID token of the calling admin).
    *   **Input (JSON):** `{ dataType: "stock" | "foodExpenses", csvData: "..." }`
    *   **Output (JSON):** Success: `{ message }`, Error: `{ error, details?, code? }`
...
*   **Action Required:** Develop a comprehensive testing strategy:
    ...
    *   **Integration Tests:**
        *   **Focus:** Interactions between several components, API calls (e.g., to `/api/admin/create-user`, `/api/admin/reset-data`), interactions with Firebase services (mocked or using emulators), and context providers.
