...
export default function ScrapeHungerboxDialog({ isOpen, onClose }: ScrapeHungerboxDialogProps) {
  ...
  const handleImport = async () => {
    ...
    if (!activeSiteId || !activeStallId) {
        toast({ title: "Context Error", description: "An active site and stall must be selected to import data.", variant: "destructive" });
        return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/scrape-hungerbox', {
        ...
        body: JSON.stringify({ username, password, siteId: activeSiteId, stallId: activeStallId }),
      });
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Enter your Hungerbox admin credentials to automatically import daily sales data. Your credentials are not stored.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleImport} disabled={isScraping || !username || !password || !activeSiteId || !activeStallId}>
