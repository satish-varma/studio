...
export const createAuthUser = functions.https.onCall(async (data: CreateAuthUserData, context: CallableContext) => {
  functions.logger.info("createAuthUser function triggered.", { structuredData: true, email: data.email, displayName: data.displayName });
  ...
  try {
    ...
    functions.logger.info(`createAuthUser: Caller UID ${callerUid} confirmed as admin. Proceeding...`);
     ...
     functions.logger.error(`createAuthUser: Error during admin role verification for UID ${callerUid}:`, err.message, { stack: err.stack });
  ...
  try {
    ...
    functions.logger.error(`createAuthUser: Error from admin.auth().createUser() for email ${email}:`, error.message, { code: error.code, stack: error.stack });
